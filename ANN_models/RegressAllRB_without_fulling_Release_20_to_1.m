...
%% 3.1 Переконвертация формата данных
pL = length(prepared_X);
pX = zeros(20 * pL, 6);
pY = zeros(20 * pL, 1);
n = 1;
for j = 1 : 20
    for i = 1 : pL
        pY(n) = prepared_Y(i, j);
        pX(n, :) = [prepared_X(i, :), i / 20];
        n = n + 1;
    end
end
% plot(1 : 20 * pL, pY, 1 : 20 * pL, prepared_Y(:) - 1.5)
prepared_X = pX;
prepared_Y = pY;
disp([datestr(datetime) ': отсчеты переконвертированы в формат [6] -> [1]']);
...
if (test_mode == false)
    ...
    save([path_out 'my_X_test.mat'], 'X_test');
    save([path_out 'my_Y_test.mat'], 'Y_test');
    save([path_out 'my_X_train.mat'], 'X_train');
    save([path_out 'my_Y_train.mat'], 'Y_train');
...
%%
% nof = 0;
% for i = 1 : length(graphic)
%     if (graphic(i) > 0.9)
%         nof = nof + 1;
%     end
% end
% nof = nof / length(graphic)